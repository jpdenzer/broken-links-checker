...
function extractLinkText($, el) {
  ...
  if (imgAlt && imgAlt.trim()) return `[image: ${imgAlt.trim()}]`.slice(0, 200);
  return '';
}
...
export async function crawlSite(startUrl, opts, emit) {
  ...
  function checkLinkOnce(link, foundOnPage, linkText) {
    if (!pageCheckCache.has(link)) {
      ...
      promise.then((result) => {
        ...
        emit('link-checked', { ...result, foundOn: foundOnPage, text: linkText });
        emit('progress', { ...stats, pagesQueued: queue.length + inFlight.size });
      });
    } else {
      pageCheckCache.get(link).then((result) => {
        emit('link-checked', { ...result, foundOn: foundOnPage, text: linkText, cached: true });
      });
    }
  ...
  async function processPage(pageUrl) {
    ...
    // url -> anchor text. The same URL can appear more than once on a page
    // (e.g. an icon-only permalink plus a separate titled link) - keep
    // scanning past an empty-text occurrence so a later, textful one wins.
    const hrefs = new Map();
    $('a[href]').each((_, el) => {
      ...
      if (!abs) return;
      if (!hrefs.has(abs)) {
        hrefs.set(abs, extractLinkText($, el));
      } else if (!hrefs.get(abs)) {
        const text = extractLinkText($, el);
        if (text) hrefs.set(abs, text);
      }
    });

    for (const [link, linkText] of hrefs) {
      let linkOrigin;
      try {
      ...
      if (sameOrigin) {
        checkLinkOnce(link, pageUrl, linkText);
        if (
